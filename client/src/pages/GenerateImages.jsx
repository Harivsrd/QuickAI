...
export function GenerateImages() {
    const imagesStyle = ['Realistic', 'Ghibli style', 'Anime style', 'Cartoon style', 'Fantasy style',
    ...
    return (
        ...
        text-slate-700">
            ...
            border-gray-200">
                ...
                <div className="mt-3 flex gap-3 flex-wrap m:max-w-9/11">
                    {imagesStyle.map((item) => (
                        <span onClick={() => setSelectedStyle(item)} key={item}
                            className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ?
                                'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`}>{item}</span>
                    ))}
                </div>

                <div className="my-6 flex items-center gap-2">
                    <label className="relative cursor-pointer">
                        <input className="sr-only peer" onChange={(e)=>setPublish(e.target.checked)} type="checkbox" name="" id="" checked={publish} />
